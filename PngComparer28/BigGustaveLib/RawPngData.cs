...
internal class RawPngData
{
    ...
    public Pixel32 GetPixel(int x, int y)
    {
        if (palette != null)
        {
            var pixelsPerByte = (8 / bitDepth);
        ...
        {
            case 1:
                return new Pixel32(first, first, first, 255, true);
            case 2:
                switch (colorType)
                {
                    case ColorType.None:
                    {
                        ...
                        return new Pixel32(value, value, value, 255, true);

                    }
                    default:
                        return new Pixel32(first, first, first, data[pixelStartIndex + 1], true);
                }

            case 3:
                return new Pixel32(first, data[pixelStartIndex + 1], data[pixelStartIndex + 2], 255, false);
            case 4:
                switch (colorType)
                {
                    case ColorType.None | ColorType.AlphaChannelUsed:
                    {
                        ...
                        return new Pixel32(gray, gray, gray, alpha, true);
                    }
                    default:
                        return new Pixel32(first, data[pixelStartIndex + 1], data[pixelStartIndex + 2], data[pixelStartIndex + 3], false);
                }
            case 6:
                return new Pixel32(first, data[pixelStartIndex + 2], data[pixelStartIndex + 4], 255, false);
            case 8:
                return new Pixel32(first, data[pixelStartIndex + 2], data[pixelStartIndex + 4], data[pixelStartIndex + 6], false);
            default:
