...
public class PngBuilder
{
    ...
    /// <summary>
    /// Create a builder from a Pixel32[ , ].
    /// </summary>
    public static PngBuilder FromPixel32_2d(Pixel32[,] pixel32s_2d, bool hasAlphaChannel)
    {
        sizeP size = new(pixel32s_2d.GetLength(0), pixel32s_2d.GetLength(1));
        var result = Create(size.Width, size.Height, hasAlphaChannel);

        for (int y = 0; y < size.Height; y++)
            for (int x = 0; x < size.Width; x++)
                result.SetPixel(pixel32s_2d[x, y], x, y);

        return result;
    }

    /// <summary>
    /// Create a builder from a Pixel32 Array
    /// </summary>
    public static PngBuilder FromPixel32(Pixel32[] pixel32s, int width, int height, bool hasAlphaChannel)
        => FromPixel32_2d(pixel32s.To2dArray(new(width, height)), hasAlphaChannel);
    ...
    public static PngBuilder FromBgra32Pixels(Stream data, int width, int height, bool useAlphaChannel = true)
    {
        ...
        for (var y = 0; y < height; y++)
            for (var x = 0; x < width; x++)
            {
                ...
                if (useAlphaChannel)
                    builder.SetPixel(new Pixel32(buffer[0], buffer[1], buffer[2], buffer[3], false), x, y);
                else
    ...
    /// </summary>
    public PngBuilder SetPixel(byte r, byte g, byte b, int x, int y) => SetPixel(new Pixel32(r, g, b), x, y);

    /// <summary>
    /// Set the pixel value for the given column (x) and row (y).
    /// </summary>
    public PngBuilder SetPixel(Pixel32 pixel, int x, int y)
    {
        ...
        rawData[start++] = (byte)pixel.R;
        rawData[start++] = (byte)pixel.G;
        rawData[start++] = (byte)pixel.B;

        if (hasAlphaChannel)
            rawData[start] = (byte)pixel.A;
    ...
    private static int PixelToColorInt(Pixel32 p) => PixelToColorInt((byte)p.R, (byte)p.G, (byte)p.B, (byte)p.A);
