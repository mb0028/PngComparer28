...
public class Png
{
    ...
    public int Height => Header.Height;
        ...
        /// <summary>
    /// The width and height of the image in pixels as <see cref="sizeP"/>.
    /// </summary>
    public sizeP SizeP => new(Header.Width, Header.Height);
    ...
    public Pixel32 GetPixel(int x, int y) => data.GetPixel(x, y);
    ...
    // /// <summary>
    // /// Read the PNG from the JPG file path by encoding it to PNG after load
    // /// </summary>
    // /// <param name="filePath">The path to the JPG file to open.</param>
    // /// <param name="settings">Settings to apply when opening the PNG.</param>
    // /// <remarks>This will open the file to obtain a <see cref="FileStream"/> so will lock the file during reading.</remarks>
    // /// <returns>The <see cref="Png"/> data from the file.</returns>
    // public static Png OpenFromJpg(string filePath)
    // {
    //     using (var fileStream = File.OpenRead(filePath))
    //     {

    //         return Open(fileStream, settings: null);
    //     }
    // }
}
